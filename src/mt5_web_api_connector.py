...
class MT5Connector:
    ...
    def get_symbols(self):
        ...
        client = MT5WebAPIClient(address=self.address, cert_validate=False)
        client.authorize(self.data_set["LOGIN_MT5"], self.data_set["WEB_API_PASSWORD_MT5"])
        response = client.get('symbol_list')
    ...
    def get_symbol_settings(self,symbol:str):
        ...
        client = MT5WebAPIClient(address=self.address, cert_validate=False)
        client.authorize(self.data_set["LOGIN_MT5"], self.data_set["WEB_API_PASSWORD_MT5"])
        response = client.get('symbol_get',{'symbol': symbol})
